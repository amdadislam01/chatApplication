...
const ProfilePage = () => {
  ...
  const [name, setName] = useState('Amdad Islam')
      ...
      <div className='w-5/6 max-w-2xl backdrop-blur-2xl text-gray-300 border-2 border-gray-600 flex items-center justify-between max-sm:flex-col-reverse rounded-lg'>
        <form onSubmit={handleSubmit} className='flex flex-col gap-5 p-10 flex-1'>
          ...
          <label htmlFor="avatar" className='flex items-center gap-2 cursor-pointer'>
            <input onChange={(e)=>setSelectedImg(e.target.value[0])} type="file"  id='avatar' accept='.png, .jpg, .jpeg' hidden/>
          ...
          <button className='bg-gradient-to-r from-pink-400 to-violet-600 text-white p-2 rounded-full text-lg cursor-pointer' type='submit'>Save</button>
        </form>
